...
from burp.utils import compute_sha256, now_utc_iso, parse_decimal, parse_competencia, find_key, safe_filename
...
def _parse_facto_date(value: object) -> str | None:
    if value is None:
        return None
...
def _parse_facto_competencia(value: object) -> str | None:
    iso_date = _parse_facto_date(value)
    if iso_date:
...
def _extract_facto_rows(html: str, table_id: str) -> list[dict[str, object]]:
    ...
    for row in table.find_all("tr"):
        ...
        if pagamentos_attr:
            try:
                ...
                logger.warning("facto detail payload could not be decoded for table=%s", table_id)

        if pagamentos:
            for pagamento in pagamentos:
                rows.append({**summary, **pagamento, "_facto_detail": pagamento})
            continue
...
def _map_facto_rows(
    ...
    for row in rows:
        ...
        detail = row.get("_facto_detail") if isinstance(row.get("_facto_detail"), dict) else None
        ...
        data_pagamento = _parse_facto_date(detail.get("DataPagamento")) if detail else None
        if not data_pagamento and data_key:
            data_pagamento = _parse_facto_date(row.get(data_key))
        competencia = _parse_facto_competencia(detail.get("DataCompetencia")) if detail else None
        if not competencia and data_key:
            competencia = _parse_facto_competencia(row.get(data_key))
...
def ingest_facto(
    ...
    if not formatted_cpf:
        update_source_run("facto_conveniar", "skipped", collected_at, "cpf_required")
        return IngestResult(
            source_id="facto_conveniar",
            status="skipped",
            records=0,
        ...
        for pagina, periodo_key, table_id, categoria in [
            ...
            for win_start, win_end in windows:
                ...
                filename = safe_filename(
                    f"facto_{pagina}_{win_start.isoformat()}_{win_end.isoformat()}.html",
                    f"facto_{pagina}.html",
                )
                ...
                raw_files += 1
                rows = _extract_facto_rows(resp.text, table_id)
                records = _map_facto_rows(
                    rows,
                    "facto_conveniar",
                    raw_id,
                    url,
