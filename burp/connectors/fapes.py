...
def _select_latest_xlsx(resources: list[dict]) -> dict | None:
    candidates = []
    for res in resources:
        ...
        candidates.append((year or 0, res))
    if not candidates:
        return None
...
def ingest_fapes() -> IngestResult:
    ...
    try:
        ...
        resource = _select_latest_xlsx(resources)
        if not resource:
        ...
        filename = filename_from_url(url, "fapes.xlsx")
        ...
        for row in iter_xlsx_rows(local_path):
            ...
            programa_key = find_key(keys, ["programa", "modalidade", "projeto"])
            ...
            record = {
                ...
                "competencia": competencia or (str(year) if year else None),
                "data_pagamento": None,
                "valor_bruto": valor,
                "descontos": None,
                "valor_liquido": valor,
        ...
        return IngestResult(source_id="fapes_bolsas", status="ok", records=count, raw_files=1)
